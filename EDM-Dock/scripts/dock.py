...
if __name__ == '__main__':
    ...
    parser.add_argument('--weight_path', type=str, help='path of saved weights', required=True)
    parser.add_argument('--config_path', type=str, help='path of config file', required=True)
    parser.add_argument('--result_path', type=str, help='path of saved results', required=True)
    ...
    weight_path = get_last_checkpoint(args.weight_path)
    ...
    model.load_state_dict(torch.load(weight_path, map_location='cuda:0')['state_dict'])
    model.eval()
